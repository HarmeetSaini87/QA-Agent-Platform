...
> All routes: `requireAuth`. All AI engines are **advisory only** — never mutate collections or runtime state.

| Method | Path | Description |
|---|---|---|
| GET | `/api/ai-intelligence/collections/:id/recommendations` | AI recommendations bundle for a collection. |
| GET | `/api/ai-intelligence/collections/:id/graph-overlay` | AI graph overlay badges (flakiness, retry hotspots, SLA breach). |
| GET | `/api/ai-intelligence/runs/:runId/rca-hints` | RCA hint list for a completed run. |
| POST | `/api/ai-intelligence/collections/:id/generate-negative-tests` | Generate negative test suite (missing fields, wrong types, boundary, auth-stripped, wrong method). Body: `{}`. Returns `NegativeTestSuite`. Audit: `api:intelligence:negative-tests:generated`. |
| POST | `/api/ai-intelligence/steps/:stepId/suggest-assertions` | Suggest assertions from a completed step result. Body: `{ runId }`. Returns `AssertionSuggestionBundle`. Audit: `api:intelligence:assertions:suggested`. |

---

## Debugger Engine (Phase F)

> All routes: `requireAuth`. Read-only — never trigger execution.

| Method | Path | Description |
|---|---|---|
| GET | `/api/api-runs/:runId/timeline` | Execution timeline events for a run. Loads stored artifact or synthesizes from snapshot. Returns `TimelineCaptureResult`. |
| GET | `/api/api-runs/:runId/variable-trace` | Per-node variable mutation trace. Returns `VariableTrace` with `mutations[]` in execution order. |
| POST | `/api/api-runs/:runId/replay-node` | Advisory node replay context. Body: `{ nodeId }`. Returns `NodeReplayContext` with variable state at entry, prior record, step request. Does **not** execute. |
| POST | `/api/api-runs/:runId/replay-workflow` | Advisory full-workflow replay plan. Body: `{ targetNodeIds?: string[] }`. Returns `WorkflowReplayPlan` with per-node decisions. Does **not** execute. |

---

## Execution Coordinator (Phase C)

> All routes: `requireAuth`. Coordinator is active when `USE_COORDINATOR=true` env var is set.

| Method | Path | Description |
|---|---|---|
| GET | `/api/coordinator/health` | Coordinator + worker health. Returns `{ coordinatorReady, useCoordinator, workerHealth, activeRuns, totalDispatched }`. |
| GET | `/api/api-runs/:runId/coordinator-state` | Run lifecycle state: `dispatched → running → completed/failed/cancelled/error`. |
| POST | `/api/api-runs/:runId/cancel` | Cancel an in-flight run. Body: `{ reason?: string }`. No-op if already completed. |
| POST | `/api/api-runs/:runId/replay-node-context` | Advisory node replay context via coordinator bridge. Body: `{ nodeId }`. Returns `NodeReplayContext`. Does **not** execute. |

---

## Plugin Ecosystem (Phase E)

> All routes: `requireAuth`.

| Method | Path | Description |
|---|---|---|
| GET | `/api/plugins` | List plugins. Query: `capability`, `tenantId`, `status`. |
| POST | `/api/plugins` | Register a plugin. Body: `PluginManifest`. |
| GET | `/api/plugins/:pluginId` | Get plugin details. |
| POST | `/api/plugins/:pluginId/enable` | Enable a plugin. |
| POST | `/api/plugins/:pluginId/disable` | Disable a plugin. Body: `{ reason? }`. |
| POST | `/api/plugins/:pluginId/hooks` | Register a hook. Body: `{ hookType, priority? }`. |
| GET | `/api/plugins/hooks/:hookType` | List hooks of a given type. |
| POST | `/api/plugins/:pluginId/check-permission` | RBAC permission check. Body: `{ actorRole, capability, tenantId? }`. |
| GET | `/api/plugins/sdk/workflow/:collectionId` | SDK read-only workflow info for a collection. |
| GET | `/api/plugins/examples` | List example plugins with manifests and usage docs (custom-bearer-auth, custom-json-assertion). |

---

## Worker Pool Health (Phase D/F)

> All routes: `requireAuth`.

| Method | Path | Description |
|---|---|---|
| GET | `/api/worker-pool/health` | Full worker pool health report. |
| GET | `/api/worker-pool/health/stuck-runs` | Stuck runs only. |
| GET | `/api/api-runtime/health` | Alias for `/api/worker-pool/health` (Phase F path). |
