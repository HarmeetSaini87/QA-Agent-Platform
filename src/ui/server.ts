...
function spawnRunWithSpec(record: RunRecord, specPath: string, headed?: boolean, retries = 0, browsers: BrowserName[] = ['chromium'], traceMode: 'on' | 'on-first-retry' = 'on-first-retry'): void {
  const { runId } = record;
...
app.post('/api/suites/:id/run', requireAuthOrApiKey, requireEditor, async (req: Request, res: Response) => {
  ...
  const record: RunRecord = {
    ...
    traceMode:       (req.body.traceMode === 'on' ? 'on' : 'on-first-retry'),
  };
  ...
  const traceArg = record.traceMode === 'on' ? 'on' : 'on-first-retry';
