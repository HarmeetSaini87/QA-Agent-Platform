...
function spawnRunWithSpec(record: RunRecord, specPath: string, headed?: boolean, retries = 0, browsers: BrowserName[] = ['chromium'], traceMode: 'on' | 'retain-on-failure' | 'off' = 'on'): void {
  const { runId } = record;
...
app.post('/api/suites/:id/run', requireAuthOrApiKey, requireEditor, async (req: Request, res: Response) => {
  ...
  const traceArg = (['on', 'retain-on-failure', 'off'].includes(record.traceMode ?? '') ? record.traceMode! : 'on') as 'on' | 'retain-on-failure' | 'off';
...
app.put('/api/flaky/config', requireEditor, (req: Request, res: Response) => {
  const { projectId, suiteId, overrides } = req.body as
    { projectId: string; suiteId?: string; overrides: Partial<FlakinessConfig> };
