...
export function registerCommonDataRoutes(app: express.Application): void {
  ...
  app.post('/api/common-data', requireAuth, requireEditor, (req: Request, res: Response) => {
    ...
    const record: CommonData = { id: uuidv4(), projectId, dataName: sanitizeInput(dataName), value: storedValue, environment, sensitive: isSensitive, createdBy: req.session.username!, createdAt: now, updatedAt: now };
